...
//    var _screenTrack = null;
   ...
   function eventBinding() {
    ...
    $("#filter").on('change',async function(event){
        ...
        _localVideoPlayer.style.filter=currentFilter
    })
   ...
   function setupMediaRecorder() {
    ...
    // if (_screenTrack) {
    //     _width = _videoCamSSTrack.getSettings().width;
    //     _height = _videoCamSSTrack.getSettings().height;
    // }
    if (_videoCamSSTrack) {
        _width = _videoCamSSTrack.getSettings().width;
        _height = _videoCamSSTrack.getSettings().height;
    ...
    //     });
        ...
        if (_videoCamSSTrack && _videoCamSSTrack.readyState === "live") {
            ...
            merger.addStream(new MediaStream([_videoCamSSTrack]), {
                x: 0,
                y: merger.height - 100,
                width: 100,
                height: 100,
    ...
    // important//////videoRecPlayer.style.filter=currentFilter;
    videoRecPlayer.load();
    $(videoRecPlayer).show();
   ...
   async function createConnection(connid) {
       console.log("mainvideowalafunction",connid);
   ...
   function applyfilter2(connId,filter){
       console.log("inside wrtchapplyfilter",connId);
        var _remoteVideoPlayer = document.getElementById('v_' + connId)
       _remoteVideoPlayer.srcObject = null;
       _remoteVideoPlayer.srcObject = _remoteVideoStreams[connId];
       _remoteVideoPlayer.style.filter=filter;
       _remoteVideoPlayer.load();
   }
   async function _createOffer(connid) {
       console.log("createoffer",connid);
   ...
   return {
       ...
       applyfilterm:function(id,filter){
          console.log("gussoooooooo");
          console.log("inside wrtch",id);
          console.log(_remoteVideoStreams);
           applyfilter2(id,filter);
       }
